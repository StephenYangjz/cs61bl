...
public class DBTable<T> {
    ...
    public <R extends Comparable<R>> List<T> getOrderedBy(Function<T, R> getter) {
        ...
        Collections.sort(Ordered, (o1, o2) -> (getter.apply(o1)).compareTo(getter.apply(o2)));
        return Ordered; // FIX ME
    }
    ...
    public <R extends Number> List<T> duplicateOn(Function<T, R> getter) {
        return getEntries().stream()
                .flatMap(a -> Stream.generate(() -> a).limit(getter.apply(a).longValue()))
                .collect(Collectors.toList()); // FIX ME
    }
