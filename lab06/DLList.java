public class DLList {
    ...
    public void insert(Object o, int position) {
        // fill me in
        if(this.size == 0) {
            sentinel = new DLNode(o, sentinel, sentinel);
            size++;
        }
        else if(position == 0) {
            ...
            DLNode point = sentinel.next;
            if(position >= size) insertBack(o);
             else {
                while (position > 1) {
                    point = point.next;
                    position--;
                }
                DLNode newone = new DLNode(o, point, point.next);
    ...
    public void reverse() {
        // fill me in
        DLNode point  = sentinel;
        DLNode l  = sentinel;
        DLNode s  = sentinel.next;
        sentinel.prev =s;
